...
func main() {
	...
	flag.StringVar(&pseudoParam.Platform, "pseudo-platform", "", "修改系统名")
	flag.StringVar(&pseudoParam.PlatformVersion, "pseudo-platform-version", "", "修改系统版本")
	flag.StringVar(&pseudoParam.Arch, "pseudo-arch", "", "修改系统架构")
	flag.StringVar(&pseudoParam.Virt, "pseudo-virt", "", "修改系统虚拟化")
	flag.StringVar(&pseudoParam.CPUModel, "pseudo-cpu-model", "", "修改CPU型号")
	flag.StringVar(&pseudoParam.CPUCoreCount, "pseudo-cpu-core", "", "修改CPU核数")
	flag.Uint64Var(&pseudoParam.MemTotal, "pseudo-mem-total", 0, "修改内存大小(Byte)")
	flag.Uint64Var(&pseudoParam.DiskTotal, "pseudo-disk-total", 0, "修改硬盘大小(Byte)")
	flag.Uint64Var(&pseudoParam.SwapTotal, "pseudo-swap-total", 0, "修改交换大小(Byte)")
	flag.StringVar(&pseudoParam.IP, "pseudo-ip", "", "修改上报IP")
	flag.StringVar(&pseudoParam.Loc, "pseudo-loc", "", "修改上报位置(ISO 3166-1 alpha-2)")
	flag.Uint64Var(&pseudoParam.BootTime, "pseudo-boot-time", 0, "修改启动时间(UnixTimeStamp)")
	flag.StringVar(&pseudoParam.Version, "pseudo-version", "pseudo-nezha-agent", "修改Agent版本")
	flag.StringVar(&pseudoParam.CPUUsed, "pseudo-cpu-used", "", "修改CPU占用")
	flag.StringVar(&pseudoParam.MemUsed, "pseudo-mem-used", "", "修改内存占用")
	flag.StringVar(&pseudoParam.SwapUsed, "pseudo-swap-used", "", "修改交换占用")
	flag.Uint64Var(&pseudoParam.TcpConnCount, "pseudo-tcp-count", 0, "修改TCP连接数")
	flag.Uint64Var(&pseudoParam.UdpConnCount, "pseudo-udp-count", 0, "修改UDP连接数")
	flag.Uint64Var(&pseudoParam.NetworkInMultiplier, "pseudo-netin-factor", 0, "传入流量倍率")
	flag.Uint64Var(&pseudoParam.NetworkOutMultiplier, "pseudo-netout-factor", 0, "传出流量倍率")
	flag.Parse()
